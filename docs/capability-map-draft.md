...
- **Subagents**: you can *watch* everything and *act* on almost nothing. A tracked subsession can be stopped only by opening it as a session and pressing the composer's Stop. A pi-subagent run **cannot be stopped or steered from anywhere** — the app literally ships a constant saying so (`C:appState.ts`).
...
|---|---|---|---|---|---|
| Open action palette | `⌘K`, context-bar "Show Actions" button | global | — | yes | C:plugins/core/actions.ts; C:components/appShell/AppContextBar.ts (Show Actions button) |
| Open Quick Switcher | `⌘P`, context-bar quick-switch button | global (all loaded sessions) | — | yes | C:components/PiWebApp.ts |
| Full page reload / hard reload | palette; refresh control | global | — | yes | C:plugins/core/actions.ts; C:components/PiWebApp.ts |
| Open settings (`⌘,`) | palette; settings route `?settings=` | global | — | yes | C:plugins/core/actions.ts; C:settingsRoute.ts |
| Select theme / follow system / UI scale | Settings → Appearance; palette theme.select | global | — | yes | C:components/settings/SettingsAppearancePanel.ts |
| Keyboard shortcut customization | Settings → Shortcuts (record/none/reset) | global | — | yes | C:components/settings/SettingsShortcutsPanel.ts |
| Go to Chat / Files / Terminal (`⌘1/2/3`) | palette; mobile tool sheet | global→current workspace | derives (needs workspace) | yes | C:plugins/core/actions.ts; C:components/appShell/AppMobileToolSheet.ts (file since removed) |
| Keyboard focus jumps (`⌘G` m/p/w/s/c) | palette + keyboard | global | — | palette | C:components/PiWebApp.ts; C:plugins/core/actions.ts |
| Reset panel sizes | palette | global | — | palette | C:components/PiWebApp.ts |
| Fleet report / update / restart machines | Settings → fleet section buttons | fleet (gateway's machine list) | explicit per machine + "all" | yes | C:api/clients.ts (fleetApi); C:components/settings/SettingsFleetSection.ts |
| Self update check / apply | pi-web status banner & status endpoints | this host | — | yes | C:api/clients.ts; C:components/PiWebApp.ts (renderSelfUpdateBanner) |
| Auth login/logout/configure provider | palette auth.login/logout → AuthDialog (method → provider → OAuth/API-key) | machine | derives (selected machine) | yes | C:plugins/core/actions.ts; C:components/PiWebApp.ts |
| Package install/update/remove (pi packages) | Settings → Packages (machine selector) | machine | derives | yes | C:api/clients.ts (piPackagesApi); C:components/settings/SettingsPackagesPanel.ts |
| Plugin enable/disable; config save | Settings → Plugins / machine config | machine | derives | yes | C:components/settings/SettingsPluginsPanel.ts; C:api/clients.ts (configApi) |
...
| Add machine | MachineList heading "+ Add machine", palette, Settings → Machines, MachineDialog | machine | explicit | yes | C:../../pi-web-plugins/machines/browser/MachineList.ts; C:plugins/core/actions.ts; C:components/settings/SettingsMachinesPanel.ts; C:components/MachineDialog.ts |
| Check health / refresh machine | row menu "Check again"; palette machine.refresh | machine | explicit row | menu | C:../../pi-web-plugins/machines/browser/MachineList.ts; C:plugins/core/actions.ts |
| Rename machine (incl. local alias) | row menu Rename… (native prompt); Settings → Machines (inline input) | machine | explicit row | menu | C:../../pi-web-plugins/machines/browser/MachineList.ts; C:components/settings/SettingsMachinesPanel.ts |
| Open machine's own PI WEB (remote) | row menu "Open PI WEB"; palette machine.open | machine | explicit row | menu | C:../../pi-web-plugins/machines/browser/MachineList.ts; C:plugins/core/actions.ts |
| Remove machine (remote only) | row menu Remove; palette machine.remove; Settings | machine | explicit row | menu | C:../../pi-web-plugins/machines/browser/MachineList.ts; C:plugins/core/actions.ts |
| Machine-wide session **cleanup** (archive idle + delete archived) | "Clean up" in Sessions heading; palette "Clean up sessions" → SessionCleanupDialog | **machine (all projects/cwds)** | **NO — machine-wide despite living in a workspace list** | yes | C:components/SessionList.ts; C:components/PiWebApp.ts; SH:apiTypes.ts:785-792 |
| Status/health per machine | MachineList row meta + activity dot | machine | explicit | read-only | C:../../pi-web-plugins/machines/browser/MachineList.ts |

### Scope: PROJECT
| Action | Surface(s) | Scope | Derives? | Touch | file:line |
|---|---|---|---|---|---|
| Browse projects of machine | nav panel (tile grid) | project | derives | yes | C:components/appShell/AppNavigationPanel.ts |
| Search projects | in-panel search field | project | derives | yes | C:components/ProjectList.ts |
| Add project (path + optional create + trust) | ProjectList heading "+ Add project"; palette project.add; ProjectDialog with directory suggestions | project | explicit | yes | C:components/ProjectList.ts; C:plugins/core/actions.ts; C:components/ProjectDialog.ts |
| Open/switch project | tile click | project | explicit | yes | C:components/ProjectList.ts |
| Close project (remove from PI WEB only) | row menu "Close" + confirm | project | explicit row | menu | C:components/ProjectList.ts |
...
| Action | Surface(s) | Scope | Derives? | Touch | file:line |
|---|---|---|---|---|---|
| Browse/search workspaces | nav panel (tile grid), search field | workspace | derives | yes | C:components/WorkspaceList.ts |
| Switch workspace | tile click | workspace | explicit | yes | C:components/WorkspaceList.ts |
| Workspace trust toggle | row menu → details → "Trusted" checkbox | workspace | explicit row | menu | C:components/WorkspaceList.ts |
| Remove workspace (provider removal run, tracked as terminal command) | row menu → "Remove workspace" + confirm; palette workspace.delete | workspace | explicit row | menu | C:components/WorkspaceList.ts; C:plugins/core/actions.ts |
| Copy workspace label / path | row menu details ⧉ buttons | workspace | explicit row | menu | C:components/WorkspaceList.ts |
| Files: browse tree / view (raw/preview) / open-in-new-window | Files panel (core plugin view) | workspace | derives | yes | C:components/WorkspaceFilesPanel.ts; C:components/WorkspaceFileViewer.ts (file since moved to the core files plugin) |
| Files: upload (picker + drag-drop), cancel/dismiss batch, review dialog | Files panel toolbar + upload dialog | workspace | derives | yes | C:components/WorkspaceFilesPanel.ts; C:api/workspaceUploads.ts |
| Files refresh (`⌘⇧F`) | panel toolbar; palette | workspace | derives | yes | C:components/WorkspaceFilesPanel.ts; C:plugins/core/actions.ts |
| Terminals: list/open/close one/close all, soft keys, copy mode, select+copy, continue-in-shell, cancel command run | Terminal panel (core plugin view) | workspace / terminal | derives (selected terminal) | yes | C:components/TerminalPanel.ts |
| Workspace goals read: list, expand, per-task progress | GoalPanel (nav panel, max-height section) | workspace | derives | read-only | C:components/GoalPanel.ts (goal list section) |
| Goal refresh | ↻ button | workspace | derives | yes | C:components/GoalPanel.ts (refresh control) |
| Goal Pause/Resume/Abandon | GoalPanel command buttons → sends `/goal-pause|/goal-resume|/goal-clear` **into the selected session** | workspace list, **session execution** | mixed (see 1c) | yes | C:components/GoalPanel.ts (command buttons); C:goalProgress.ts; C:components/PiWebApp.ts |
| Archive a goal (workspace record → `archived/`) | two-press "Archive goal" | workspace | explicit goal | yes | C:components/GoalPanel.ts (archive flow); C:api/clients.ts (archiveWorkspaceGoal) |

### Scope: SESSION
| Action | Surface(s) | Scope | Derives? | Touch | file:line |
|---|---|---|---|---|---|
| Start session (`⌘⏎`, `⌘⇧N`) | Sessions heading "+", QuickSwitcher create row, palette | session | derives (selected workspace) | yes | C:components/SessionList.ts; C:components/QuickSwitcher.ts; C:plugins/core/actions.ts |
| Open session | row/tile click in list or switcher; Enter in switcher opens first match | session | explicit | yes | C:components/SessionList.ts; C:components/QuickSwitcher.ts |
| Search sessions | in-list search; QuickSwitcher search + project/workspace filter chips | session | derives | yes | C:components/SessionList.ts; C:components/QuickSwitcher.ts |
| Rename session | row menu (native `prompt()`); QuickSwitcher inline form; context-bar inline rename | session | explicit row | menu | C:components/SessionList.ts; C:components/QuickSwitcher.ts; C:components/appShell/AppContextBar.ts |
| Pin to top (**client-local, QuickSwitcher only**) | switcher row menu / long-press menu | session | explicit | menu | C:components/QuickSwitcher.ts |
| Mark read | row menu; bulk toolbar | session | explicit row | menu | C:components/SessionList.ts |
| Archive | row menu; bulk toolbar; palette session.archive | session | explicit | menu/yes | C:components/SessionList.ts; C:plugins/core/actions.ts |
| Archive with descendants | row menu + confirm | session tree | explicit | menu | C:components/SessionList.ts |
| Bulk archive / bulk delete archived / bulk mark-read | long-press→multi-select, ☑ heading toggles, toolbars | sessions (explicit set) | explicit | yes (long-press) | C:components/SessionList.ts |
| Restore archived session | row menu | session | explicit | menu | C:components/SessionList.ts |
| Delete transient "new" session | row menu; palette session.delete | session | explicit | menu | C:components/SessionList.ts; C:plugins/core/actions.ts |
| Delete archived session (permanent, confirmed) | row menu; bulk toolbar | session | explicit | menu | C:components/SessionList.ts |
| Detach from parent | row menu only | session | explicit | menu | C:components/SessionList.ts |
| Reload from disk | row menu (disabled while active); palette session.reload | session | explicit | menu | C:components/SessionList.ts; C:plugins/core/actions.ts |
| History & branches (tree navigator: expand, select, **fork**, cancel summarization) | row menu "History and branches" → dialog; also `/tree` | session tree | explicit | menu | C:components/SessionList.ts; C:components/SessionTreeNavigator.ts |
| Stop active work (`⌘.`) | composer stop button; palette session.stop | session (selected) | derives | yes | C:components/PromptEditor.ts; C:plugins/core/actions.ts; C:components/PiWebApp.ts |
| Cycle model / thinking (`/model`, thinking dialog) | composer status buttons; palette model.select/thinking.select | session | derives | yes | C:components/PromptEditor.ts; C:components/ModelPicker.ts |
| Enable/disable models in catalog | ModelPicker catalog tab toggles | session | derives | yes | C:components/ModelPicker.ts; C:api/clients.ts |
| Dismiss session warning | warnings strip | session | derives | yes | C:components/ChatView.ts |
| Subtree expand/collapse of children | chevron in session row | session tree | explicit row | yes | C:components/SessionList.ts |

### Scope: MESSAGE (selected session's transcript)
| Action | Surface(s) | Scope | Derives? | Touch | file:line |
|---|---|---|---|---|---|
| Send / **steer while streaming** (same button) | composer send button; `⏎` | session | derives | yes | C:components/PromptEditor.ts; C:components/PiWebApp.ts |
| Stop & return queue to composer | composer stop | session | derives | yes | C:components/PromptEditor.ts; C:components/PiWebApp.ts |
| Clear queue (without stopping) | queued strip "Clear queue" | session | derives | yes | C:components/ChatView.ts |
| Recall one queued message (↩) | queued bubble button | message | explicit | yes | C:components/ChatView.ts |
| Resend / "edit and send again" (↻) | bubble hover action → refills composer | message | explicit | yes | C:components/ChatView.ts; C:resendMessage.ts |
| Copy message (⧉) | bubble action | message | explicit | yes | C:components/ChatView.ts |
| Prompt history (recall past prompt) | composer history button → sheet with search | session | derives | yes | C:components/PromptEditor.ts; C:components/PromptHistoryPanel.ts |
| Attachments (picker/paste/drop), remove, zoom | composer; image parts | message | derives | yes | C:components/PromptEditor.ts; C:components/ChatView.ts |
| Shell mode (`!`), autocomplete (`/`, `@`) | composer editor | message | derives | yes | C:inputModes.ts; C:components/PromptEditor.ts |
| Dictation (when configured) | composer mic button; tap-to-talk | message | derives | yes | C:components/PromptEditor.ts |
| Load earlier messages | button / scroll-up | session | derives | yes | C:components/ChatView.ts |
| Expand message meta | meta line toggle | message | explicit | yes | C:components/ChatView.ts |
| Answer `ask_user` (multi-step form, keep editing, send anyway) | AskUserCard | session | derives | yes | C:components/AskUserCard.ts |
| Answer/cancel extension dialogs (+ settled cards) | ExtensionDialogCard | session | derives | yes | C:components/ExtensionDialogCard.ts (wiring C:components/ChatView.ts) |
| Copy/dismiss one notification; dismiss all; per-notification overflow info | drawer notifications tab | session inbox | derives | yes | C:components/ChatView.ts |
| Jump to bottom | floating button | session | derives | yes | C:components/ChatView.ts |

### Scope: SUBAGENT RUN / CHILD WORK (selected session's drawer)
| Action | Surface(s) | Scope | Derives? | Touch | file:line |
|---|---|---|---|---|---|
| Open tracked subsession (selects it as a session) | drawer "Subagent" row | child session | explicit row | yes | C:components/ChatView.ts; C:components/PiWebApp.ts |
| **Stop a tracked subsession** | only indirectly: open it, then composer Stop | child session | explicit (after open) | indirect | C:components/PiWebApp.ts + C:components/PromptEditor.ts |
| Open a pi-subagent run's conversation (read-only) | drawer "Agent" row → dialog; states why steering is unavailable | subagent run | explicit row | yes | C:components/ChatView.ts; C:appState.ts |
| Open background task output | drawer "Task" row (disabled until output exists) | background task | explicit row | yes | C:components/ChatView.ts; C:components/PiWebApp.ts |
| Activity filter (kind) / active-vs-finished scope | drawer chips | session | derives | yes | C:components/ChatView.ts |
| **Steer / interrupt / stop / resume a pi-subagent run** | **NOWHERE** — constant `SUBAGENT_INTERVENTION_UNAVAILABLE` | subagent run | — | — | C:appState.ts |
| Cancel a queued terminal-command run | terminal command-run card | terminal command | explicit | yes | C:components/TerminalPanel.ts |
...
5. **`machineId = "local"` defaults everywhere** (`C:api/clients.ts`, `sessionSocket.ts:44`, and component props `PromptEditor.ts:211`, `TerminalPanel.ts:29`, `ProjectDialog.ts:24`, `WorkspaceList.ts:30`): today's call sites pass `selectedMachineId(state)` (138 references), so it's a structural foot-gun rather than an active bug — one forgotten argument silently targets the local machine.
...
**Today's state.** The web UI renders child work in the chat drawer: *Subagent* rows (tracked subsessions, `ChatView.ts:1495-1509`) and *Agent* rows (pi-subagent tool runs, `:1512-1532`). Both are open-only buttons. A run's conversation opens read-only with the banner "Steering this run is not available from the web app." — the exported constant `SUBAGENT_INTERVENTION_UNAVAILABLE` (`C:appState.ts`, used at `:61`). The justification comment (`appState.ts:31-36`) says steering "travels over the subagent extension's RPC on the in-process Pi event bus, which this server does not hold." **That premise is now false**: the daemon *is* the process hosting the agent, and the event bus is injectable.
...
1. **File management in the Files panel.** Endpoints and the plugin API exist for delete/move/write (`clients.ts` workspacesApi; `C:plugins/workspaceFiles.ts`), but the core panel offers only browse, view (raw/preview), upload, refresh (`WorkspaceFilesPanel.ts:69-70,106,152`). Renaming or deleting a file requires a terminal.
...
|---|---|---|---|---|---|---|
| QuickSwitcher tiles | `min-height: 52px`, **no fixed height**; rows auto → tiles differ by row | **1-line ellipsis desktop; 2-line ≤420px** | subtitle 1-line ellipsis | `minmax(240px,1fr)` → **`minmax(140px,1fr)` on phones** | 52px row; menu btn 32px | C:components/QuickSwitcher.ts |
| ProjectList / WorkspaceList tiles | `min-height: 56px` + **`grid-auto-rows: min-content` + `align-self: start`** → rows are each as tall as their tallest tile | **2-line clamp, break-all** | Workspace: only when label items exist; Project: path always | `minmax(150px,1fr)` | menu btn 32px (36px coarse) | C:components/shared.ts |
...
| SessionList rows | no min-height on `.action-main` (list mode) | 2-line clamp (`max-height: 2.5em`) | `<small>` meta **always** (status · N messages) | rows, not tiles | heading buttons **30px** (36px mobile); menu 32px (36px) | C:components/shared.ts; C:components/SessionList.ts |
| MachineList rows | `min-height: 58px` (own override) | 1-line ellipsis | always | rows | 58px | C:../../pi-web-plugins/machines/browser/MachineList.ts |
| GoalPanel cards | `min-height: 40px` header (42px narrow) | 1-line ellipsis objective | meta row optional (collapsed) | cards stacked | 40/42px | C:components/GoalPanel.ts (archive flow) |
| ChatView activity rows (subagent/run/task) | `min-height: 38px` | 1-line ellipsis per column | optional detail line | single column | 38px | C:components/ChatView.ts |
| Notification rows | **no min-height**; padding only | wraps free | always | single column | controls 32px | C:components/ChatView.ts |
...
2. **One column metric.** `repeat(auto-fit, minmax(150px, 1fr))` everywhere a tile grid appears; delete the QuickSwitcher 240/140 pair (or move them onto the same token; the MachineSwitcher pair died with the component in b0bce2a0).
...
**Exact violations to fix against those rules:** `QuickSwitcher.ts` (tile grid, clamp split - line numbers drift), `shared.ts:252-254` (`grid-auto-rows: min-content`, `align-self: start`). (Historical note: the deleted MachineSwitcher's 140/6px pair was part of this list; the component is gone since b0bce2a0.), `shared.ts:259` (2-line, break-all — keep as the rule), `MachineSwitcher.ts (deleted)` (third grid), `MachineList.ts:243` (58px row), `ChatView.ts:188,231` (38px/30px rows), `SessionList.ts:665` + `GoalPanel.ts:227` (30px heading buttons), `ChatView.ts:263` (32px controls).
