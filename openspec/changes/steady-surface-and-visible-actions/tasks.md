...
- [ ] 3.4 Re-run the probe after 3.3 lands and record the GREEN numbers (all sampled deltas 0 px; tap activates first touch). Current standing (before 3.3): all geometry deltas already 0 px, 30 samples × 2 screens; the single-tap leg was separately measured live (synthesizeTapGesture → clicksOnButton=1, dialog closed). Evidence: full probe output in the change notes; this is the acceptance gate for the pending-input-stability spec.
