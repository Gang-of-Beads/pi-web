...
VISUAL LEG CLOSED: the deep-shadow walk found the d7-live exchange rendered in the transcript (2 hits) and NO delivery-mark anywhere — correct by design: a settled message hides its marker (the double-tick-forever bug this avoids). The ◌→✓→✓✓→hidden transition is real (unit-pinned per state, the frame on the wire) and ephemeral by design; the evidence chain is complete: unit pins + the frame published + the settled exchange rendered. The reconcile removal (4.1) now proceeds on D8's review. Original: e2e on 8505 at each landing
...
- MECHANISM RESOLVED (2026-08-30): the task registry is FILE-BASED — the extension writes .json records, the daemon reads on demand; there are no in-daemon transition events to hook. SIMPLER PATH DISCOVERED THE SAME DAY: the 2s heartbeat's refreshBackgroundRunCounts ALREADY detects background-run count changes and publishes status.update (sequenced, repaired) to the client — the frames exist. The client slice: on a status.update whose background run count differs from the last seen, trigger ONE refreshSubagents (an on-demand refetch instead of the 4s timer). The fs.watch idea is withdrawn — the sequenced status frames already carry the signal.
