...
VISUAL LEG UPDATE (same evening): the composer send WORKED via the CM view dispatch (Reflect into the editor handle) + the send button click — the draft cleared and the message entered the transcript (the page title became the message text). The delivery-marker read is still limited by the virtualized window mid-turn; the ◌→✓ transition is unit-pinned (commandStateLabel) and the frame is live server-side. A settled-turn read or accepting the unit+wire evidence closes this leg. Original: e2e on 8505 at each landing
