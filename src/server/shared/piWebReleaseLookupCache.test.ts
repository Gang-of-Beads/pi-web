...
describe("createPiWebReleaseLookupCache", () => {
  it("serves a fresh cached release lookup", async () => {
    ...
    const lookup = createPiWebReleaseLookupCache(load, { ttlMs: 100, now: () => now });

    await expect(lookup("0.9.0")).resolves.toMatchObject({ latestVersion: "1.0.0", checkedAtMs: 1_000 });
    now = 1_050;
    await expect(lookup("0.9.1")).resolves.toMatchObject({ latestVersion: "1.0.0", checkedAtMs: 1_000 });
  ...
  it("bypasses a fresh lookup when forced", async () => {
    ...
    await expect(lookup("0.9.0", { force: true })).resolves.toMatchObject({ latestVersion: "1.1.0", checkedAtMs: 1_050 });
    await expect(lookup("0.9.0")).resolves.toMatchObject({ latestVersion: "1.1.0", checkedAtMs: 1_050 });
  ...
  it.each(["forced-first", "regular-first"] as const)("does not let an older regular lookup replace a forced result when %s completes", async (completionOrder) => {
    ...
    const lookup = createPiWebReleaseLookupCache(load);

    const regularLookup = lookup("0.9.0");
    const forcedLookup = lookup("0.9.0", { force: true });
    ...
    await expect(lookup("0.9.0")).resolves.toMatchObject({ latestVersion: "2.0.0" });
    expect(load).toHaveBeenCalledTimes(2);
  });
