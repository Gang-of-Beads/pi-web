...
describe("pinned Safe Tunnel frpc manifest", () => {
  it("pins the locally validated frp release executable for the explicit supported target", () => {
...
describe("HttpSafeTunnelFrpcArtifactSource", () => {
  it("downloads bounded bytes from a loopback fixture without exposing transport details", async () => {
    ...
    const large = await captureError(source.download(
      artifactFixture("1.0.0", expected, `${server.origin}/large`).artifact,
    ));
    expect(large).toMatchObject({ code: "download_too_large" });
  });
});
...
describe("SafeTunnelFrpcManager", () => {
  ...
  it("rejects malformed archives, non-regular target entries, and checksum mismatches without installing them", async () => {
    ...
    const malformedSource = new FixtureArtifactSource([
      [valid.artifact.downloadUrl, Buffer.from("not a gzip archive")],
    ]);
    await expect(managerFor(valid.manifest, malformedSource).ensureManagedFrpc())
    ...
    await expect(managerFor(
      valid.manifest,
      new FixtureArtifactSource([[valid.artifact.downloadUrl, symlinkArchive]]),
    ...
    await expect(managerFor(
      valid.manifest,
      new FixtureArtifactSource([[valid.artifact.downloadUrl, tamperedArchive]]),
  ...
  it("rejects unsafe or ambiguous injected manifests with one redacted application error", () => {
    ...
    const invalid: SafeTunnelFrpcManifest = {
      desiredVersion: "1.0.0",
      releases: [{
        ...fixture.release,
        artifacts: [{ ...fixture.artifact, archiveEntryPath: "../frpc" }],
      }],
    };
...
function artifactFixture(
  ...
  return {
    archive: options.archive ?? tarGzipFixture([{ path: archiveEntryPath, contents: executable }]),
    artifact,
    release,
