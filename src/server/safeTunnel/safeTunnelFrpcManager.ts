...
const defaultMaximumDownloadBytes = 64 * 1024 * 1024;
...
/**
 * Owns selection, integrity checks, and immutable-version installation. Process
 * lifecycle code consumes only the verified absolute executable path returned here.
 */
export class SafeTunnelFrpcManager implements SafeTunnelManagedFrpcProvider {
  ...
  private async ensureOnce(): Promise<SafeTunnelManagedFrpc> {
    ...
    const desiredPath = this.executablePath(desiredRelease, desiredArtifact);
    if (await this.isVerifiedExisting(desiredPath, desiredArtifact)) {
    ...
    try {
      ...
      await this.installAtomically(desiredRelease, desiredArtifact, executable);
      if (!await this.isVerifiedExisting(desiredPath, desiredArtifact)) {
  ...
  private async downloadAndVerify(artifact: SafeTunnelFrpcArtifact): Promise<Uint8Array> {
    ...
    const executable = extractExecutable(archive, artifact);
  ...
  private async useFallbackOrThrow(
    ...
    for (const release of this.manifest.releases) {
      ...
      const path = this.executablePath(release, artifact);
      if (!await this.isVerifiedExisting(path, artifact)) continue;
      return {
  ...
  private async isVerifiedExisting(
    path: string,
  ...
  private async installAtomically(
    ...
    try {
      for (const directory of [
        ...
        if (this.platform !== "win32") await chmod(directory, safeTunnelFrpcDirectoryMode);
      }
      ...
      await replaceInstalledFile(temporaryPath, finalPath, artifact, (path, expected) => (
        this.isVerifiedExisting(path, expected)
      ));
    } catch (error: unknown) {
      if (error instanceof SafeTunnelFrpcAcquisitionError) throw error;
      throw new SafeTunnelFrpcAcquisitionError("install_failed");
...
function matchesExecutable(
  ...
  return executable.byteLength === artifact.executableSize
...
function validateArtifact(artifact: SafeTunnelFrpcArtifact): void {
  ...
  if (!sha256Pattern.test(artifact.executableSha256)) throw new Error("invalid checksum");
