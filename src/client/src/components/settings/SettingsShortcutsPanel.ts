...
export class SettingsShortcutsPanel extends LitElement {
  ...
  static override styles = [interactiveSurfaceStyles, settingsControlStyles, css`
    ...
    .prompt-enter-option { display: grid; grid-template-columns: auto minmax(0, 1fr); gap: var(--pi-space-4); align-items: center; color: var(--pi-text); }
    .prompt-enter-option input { box-sizing: border-box; width: var(--pi-checkbox-size); height: var(--pi-checkbox-size); margin: 0; padding: 0; border: 0; background: transparent; accent-color: var(--pi-accent); font-family: inherit; }
    ...
    .shortcut-status small.conflict { border: 1px solid currentColor; border-radius: var(--pi-radius-pill); padding: var(--pi-space-1) var(--pi-space-4); }
