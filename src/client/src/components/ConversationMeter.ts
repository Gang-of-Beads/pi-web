...
export class ConversationMeter extends LitElement {
  ...
  static override styles = css`
    :host { position: absolute; top: -4px; left: 16px; right: 16px; z-index: 6; display: block; height: 12px; opacity: .58; transition: opacity var(--pi-motion-fast) var(--pi-ease); }
    :host(:focus-within) { opacity: .92; }
    @media (hover: hover) { :host(:hover) { opacity: .92; } }
    .meter { height: 100%; }
    .track { position: relative; height: var(--pi-dot-xs); margin-top: 4px; border-radius: var(--pi-radius-pill); background: color-mix(in srgb, var(--pi-border-muted) 34%, transparent); box-shadow: 0 0 0 1px color-mix(in srgb, var(--pi-bg) 55%, transparent); }
