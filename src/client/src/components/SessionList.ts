...
export class SessionList extends LitElement implements KeyboardNavigableSection {
              ...
              ${session.archived === true
                ...
                : canDeleteTransient
                  ...
                  : html`
                    ...
                    ${this.onOpenTree === undefined || session.cwdMissing === true ? null : html`<button title="Browse this session's history and branches" @click=${() => { this.openMenuSessionId = undefined; void this.onOpenTree?.(session); }}>History and branches</button>`}
  ...
  .bulk-select-entry { box-sizing: border-box; flex: 0 0 auto; display: inline-grid; place-items: center; width: var(--pi-control-height); height: var(--pi-control-height); padding: 0; font-size: var(--pi-text-sm); line-height: 1; text-transform: none; }
    .start-session-button { box-sizing: border-box; flex: 0 0 auto; display: inline-flex; align-items: center; justify-content: center; gap: var(--pi-space-2); min-width: var(--pi-control-height); height: var(--pi-control-height); padding: 0 var(--pi-space-4); line-height: var(--pi-control-height); }
    ...
    /* This list renders inside the navigation panel's chrome no-select rule;
       its own copyable controls must re-enable explicitly or they inherit
    ...
    /* On the row, like every other state: .action-main has no border, so the
       old descendant selector matched and painted nothing. */
    .action-row.bulk-selected { border-color: var(--pi-accent); }
...
.session-checkbox { accent-color: var(--pi-accent); position: absolute; top: 50%; transform: translateY(-50%); left: calc(var(--pi-row-gutter-start) + var(--depth, 0) * var(--pi-space-7)); z-index: 3; box-sizing: border-box; width: var(--pi-checkbox-size); height: var(--pi-checkbox-size); margin: 0; }
    ...
    .session-search-input { box-sizing: border-box; flex: 1 1 auto; min-width: 0; height: var(--pi-control-height-comfort); border: 1px solid var(--pi-border); border-radius: var(--pi-radius-md); background: var(--pi-surface); color: var(--pi-text); padding: 0 var(--pi-space-5); font: var(--pi-control-font-size, 14px) var(--pi-control-font-family, system-ui, sans-serif); line-height: inherit; }
