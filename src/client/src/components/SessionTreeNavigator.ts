...
export class SessionTreeNavigator extends LitElement {
  ...
  static override styles = [css`${unsafeCSS(uiIconStyle)}`, css`${unsafeCSS(disclosureIconStyle)}`, interactiveSurfaceStyles, css`
    ...
    * { box-sizing: border-box; }
    ...
    .entry { min-width: 0; display: flex; align-items: baseline; gap: var(--pi-space-4); }
    ...
    textarea { box-sizing: border-box; width: 100%; resize: vertical; min-height: 94px; border: 1px solid var(--pi-border); border-radius: var(--pi-radius-md); background: var(--pi-bg); color: var(--pi-text); padding: var(--pi-space-5) var(--pi-space-5); font: var(--pi-control-font-size, 16px) var(--pi-control-font-family, system-ui, sans-serif); line-height: inherit; }
