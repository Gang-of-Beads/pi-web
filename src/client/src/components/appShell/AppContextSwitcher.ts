...
export class AppContextSwitcher extends LitElement {
  ...
  static override styles = css`
    ...
    .chip-label { color: var(--pi-muted); font-size: var(--pi-text-2xs); text-transform: uppercase; }
    .chip, .add { font: var(--pi-text-sm) var(--pi-font-ui); line-height: inherit; }
    ...
    /* When a step is too narrow for label and value, the label goes first:
       ...
       says its own name.
       The value is one size per pointer, not per container: the projects nav
       renders the chips at ~103px and the sessions drawer at ~150px, and the
       container-width step-down made the same control change size when the
       reader moved between them - the "切了之后变大变小" report. On coarse
       pointers the fallback words (7-9 monospace characters) still fit the
       narrowest step at 2xs. */
    @container (max-width: 140px) { .chip-label { display: none; } }
    @media (pointer: coarse) { .chip-value { font-size: var(--pi-text-2xs); } }
