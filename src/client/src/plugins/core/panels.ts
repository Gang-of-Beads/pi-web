...
function renderTerminal(context: WorkspacePanelContext): TemplateResult {
  loadTerminalPanel();
  return html`<terminal-panel .workspace=${context.workspace} .machineId=${context.machine.id} .sessions=${context.terminal.sessions} .selectedTerminalId=${context.selectedTerminalId} .autoStart=${context.terminalAutoStart} .expanded=${context.host.workspacePanelFullscreen()} .onSelectTerminal=${context.onSelectTerminal}></terminal-panel>`;
}
