...
export function createPluginRuntimeContext(statePatch: Partial<AppState> = {}) {
  ...
  const context: PluginRuntimeContext = {
    ...
    createProject: vi.fn(() => Promise.resolve(undefined)),
    projectDirectories: vi.fn(() => Promise.resolve([])),
    projectTrust: vi.fn(() => Promise.resolve({ path: "", decision: null, trusted: false })),
    removeMachine: vi.fn((machineId: string) => { calls.push(`removeMachine:${machineId}`); }),
    refreshMachine: vi.fn((machineId: string) => { calls.push(`refreshMachine:${machineId}`); }),
    openMachine: vi.fn((machineId: string) => { calls.push(`openMachine:${machineId}`); }),
