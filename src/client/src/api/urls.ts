...
/**
 * One subagent run's conversation. Paged like a session's own transcript,
 * because a long-running child writes as much as a session does.
 */
export function subagentRunMessagePath(session: SessionLookup, runId: string, options?: { limit?: number; before?: number }, machineId = "local"): string {
  const params = new URLSearchParams();
  const cwd = sessionCwd(session);
  if (cwd !== undefined && cwd !== "") params.set("cwd", cwd);
  if (options?.limit !== undefined) params.set("limit", String(options.limit));
  if (options?.before !== undefined) params.set("before", String(options.before));
  const query = params.toString();
  return `api/machines/${encodeURIComponent(machineId)}/sessions/${encodeURIComponent(sessionId(session))}/subagent-runs/${encodeURIComponent(runId)}/messages${query === "" ? "" : `?${query}`}`;
}
