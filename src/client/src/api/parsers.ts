...
export function parseSessionsRevisionResponse(value: unknown): SessionsRevisionResponse {
  ...
  if (record["unchanged"] === true) return { revision, unchanged: true };
  return { revision, sessions: arrayOf(parseSessionInfo)(record["sessions"]) };
}
...
function parseDialogScreenLine(value: unknown): string {
  if (typeof value !== "string") throw new Error("Invalid extension dialog screen line");
...
/**
 * Presence per plugin-backed surface, or undefined when the daemon said
 * nothing.
 *
 * Undefined has to survive intact: it is how a browser learns that nobody has
 * established whether a surface exists, and a panel is kept rather than hidden
...
export function parseRealtimeStreamEvent(value: unknown): RealtimeStreamEvent {
  ...
  switch (record["type"]) {
    ...
    case "workspace.changed":
      return { type: "workspace.changed", cwd: requireNonEmptyString(record, "cwd") };
    default:
