var meta = { name: 'goal_round_d_layout', description: 'Goal convergence Round D (post-fix verification): three glm lanes audit layout coordination (alignment, whitespace rhythm, button boundaries, centering, balance) on the live 8505 stack' };
...
var CONTEXT = [
  ...
  'The app is in "pro native" style: monospace UI face, near-black space-gray palette (#0b0d10 base, lightness-stepped surfaces), square corners 0-4px, flat (no shadows), hairline borders. Goal: layout COORDINATION convergence. This is Round D - verifying the Round C fix wave (commit dc41e086): empty-session box-sizing + centering, tasks viewer flex (empty state centered), solid muted msg-header dividers, the compact pressed-state family un-nested, session-row/drawer-control/goals-refresh/palette pressed states, context-bar breathing, contributed-section inset. VERIFY each reached the reader, then hunt. Prior findings: docs/design/research/roundc-lane-{a,b,c}.md. The bar: ZERO new TRUE findings beyond the owner-deferred ledger (docs/design/review-triage-goal-round-b.md).',
  '',
  ...
  '',
  'Every finding: file:line of the owning style, the two surfaces or elements compared (or the one element + its container), measured numbers, screenshot path (/tmp/roundd-*.png), and an explicit TRUE/FALSE adjudication. If a hunt item is clean, say so with the measurement that proves it.',
].join('\n');

var laneA = runs.run('lane-a', {
  agent: 'design-reviewer-d', timeoutMs: 5400000, label: 'roundA glm A phone', output: '/tmp/roundd-lane-a.md',
...
var laneB = runs.run('lane-b', {
  agent: 'design-reviewer-e', timeoutMs: 5400000, label: 'roundA glm B desktop+responsive', output: '/tmp/roundd-lane-b.md',
...
var laneC = runs.run('lane-c', {
  agent: 'qwen-parity-reviewer', model: 'botim-bllm/glm-5.3-flash', timeoutMs: 5400000, label: 'roundd glm C tokens+balance', output: '/tmp/roundd-lane-c.md',
