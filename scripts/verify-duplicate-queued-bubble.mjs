 ...
 * Output: .pi/duplicate-queued-bubble.json
 */
...
const clientMessageIdC = "cm-record-c";
...
// The reported shape: the same words sent twice while the agent is busy. If
// the queue pairs entries to client ids by text, two identical texts are
// interchangeable and one of them can end up with no id - and a queue entry
// with no id draws a second bubble beside the one the browser already has.
await post(`/sessions/${sessionId}/prompt`, { cwd, text: "Then say DONE.", streamingBehavior: "followUp", clientMessageId: clientMessageIdB });
await post(`/sessions/${sessionId}/prompt`, { cwd, text: "Then say DONE.", streamingBehavior: "followUp", clientMessageId: clientMessageIdC });
...
for (const id of [clientMessageIdB, clientMessageIdC]) {
  if (!queuedSeen.some((entry) => entry.endsWith(id))) throw new Error(`FAIL: the queue never carried ${id}, so the browser would draw a second bubble for it`);
}
if (false) throw new Error("FAIL: prompt B was never observed in the queue carrying its id, so the queued path was not exercised");
