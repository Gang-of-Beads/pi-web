var meta = { name: 'goal_round_a_layout', description: 'Goal convergence Round A: three glm lanes audit layout coordination (alignment, whitespace rhythm, button boundaries, centering, balance) on the live 8505 stack' };
...
var CONTEXT = [
  ...
  'The app is in "pro native" style: monospace UI face, near-black space-gray palette (#0b0d10 base, lightness-stepped surfaces), square corners 0-4px, flat (no shadows), hairline borders. Goal: layout COORDINATION convergence.',
  '',
  ...
  '',
  'Every finding: file:line of the owning style, the two surfaces or elements compared (or the one element + its container), measured numbers, screenshot path (/tmp/rounda-*.png), and an explicit TRUE/FALSE adjudication. If a hunt item is clean, say so with the measurement that proves it.',
].join('\n');

var laneA = runs.run('lane-a', {
  agent: 'design-reviewer-d', timeoutMs: 5400000, label: 'roundA glm A phone', output: '/tmp/rounda-lane-a.md',
...
var laneB = runs.run('lane-b', {
  agent: 'design-reviewer-e', timeoutMs: 5400000, label: 'roundA glm B desktop+responsive', output: '/tmp/rounda-lane-b.md',
...
var laneC = runs.run('lane-c', {
  agent: 'qwen-parity-reviewer', timeoutMs: 5400000, label: 'roundA qwen tokens+balance', output: '/tmp/rounda-lane-c.md',
