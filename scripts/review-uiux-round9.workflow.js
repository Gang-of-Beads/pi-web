...
const lanePrompt = (focus, files, surfaces, out) => [
  'You are one lane of ROUND NINE of a visual-polish convergence audit of the PI WEB client (Lit web app).',
...
const laneA = () => runs.run('r9-lane-a', {
  ...
  task: lanePrompt(
    ...
    '/tmp/uiux-r9-lane-a.md'
  ),
  output: '/tmp/uiux-r9-lane-a.md'
});
const laneB = () => runs.run('r9-lane-b', {
  ...
  task: lanePrompt(
    ...
    '/tmp/uiux-r9-lane-b.md'
  ),
  output: '/tmp/uiux-r9-lane-b.md'
});
const laneC = () => runs.run('r9-lane-c', {
  ...
  task: lanePrompt(
    ...
    SURFACES,
    '/tmp/uiux-r9-lane-c.md'
  ),
  output: '/tmp/uiux-r9-lane-c.md'
});

const results = await Promise.all([laneA(), laneB(), laneC()].map(p => p.catch(e => ({ error: String(e) }))));
return { lanes: results.map((r, i) => ({ lane: i + 1, ok: !(r && r.error), error: r && r.error ? r.error : undefined })), files: ['/tmp/uiux-r9-lane-a.md', '/tmp/uiux-r9-lane-b.md', '/tmp/uiux-r9-lane-c.md'] };
