var meta = { name: 'goal_round_e_layout', description: 'Goal convergence Round E (final delivery verification): three glm lanes audit layout coordination (alignment, whitespace rhythm, button boundaries, centering, balance) on the live 8505 stack' };
...
var CONTEXT = [
  ...
  'The app is in "pro native" style: monospace UI face, near-black space-gray palette (#0b0d10 base, lightness-stepped surfaces), square corners 0-4px, flat (no shadows), hairline borders. Goal: layout COORDINATION convergence. This is Round E - the FINAL delivery verification. Round D found the dist plugin bundle was stale (built before the fix commit); it has been rebuilt and redeployed (commit 9fd3f926). VERIFY: (1) the tasks viewer empty state now centers (align-content stretch served), (2) goals refresh pressed state live, (3) shared list rows (project/workspace/machine) pressed state live in nav panel AND context sheet, (4) rail header breathing symmetric with context bar, (5) queued-message divider solid. Then the full clean sweep. The bar: ZERO new TRUE findings beyond the owner-deferred ledger (docs/design/review-triage-goal-round-b.md). A single new TRUE means the loop continues.',
  '',
  ...
  '',
  'Every finding: file:line of the owning style, the two surfaces or elements compared (or the one element + its container), measured numbers, screenshot path (/tmp/rounde-*.png), and an explicit TRUE/FALSE adjudication. If a hunt item is clean, say so with the measurement that proves it.',
].join('\n');

var laneA = runs.run('lane-a', {
  agent: 'design-reviewer-d', timeoutMs: 5400000, label: 'roundA glm A phone', output: '/tmp/rounde-lane-a.md',
...
var laneB = runs.run('lane-b', {
  agent: 'design-reviewer-e', timeoutMs: 5400000, label: 'roundA glm B desktop+responsive', output: '/tmp/rounde-lane-b.md',
...
var laneC = runs.run('lane-c', {
  agent: 'qwen-parity-reviewer', model: 'botim-bllm/glm-5.3-flash', timeoutMs: 5400000, label: 'rounde glm C tokens+balance', output: '/tmp/rounde-lane-c.md',
