...
try {
  ...
  const out = await page.evaluate(async () => {
    ...
    for (let w = 0; w < 30_000; w += 500) {
      ...
      if (text.includes("you stopped it")) return `ok: ${text.slice(text.indexOf("Model response failed"), text.indexOf("Model response failed") + 160)}`;
    }
    const text = deep(app.shadowRoot).replace(/\s+/gu, " ");
    const at = text.lastIndexOf("Model response failed");
    return `no cause in the row: ${at === -1 ? "no failure row" : text.slice(at, at + 160)}`;
  });
  console.log(out);
