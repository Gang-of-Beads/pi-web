...
const lanePrompt = (focus, files, surfaces, out) => [
  'You are one lane of ROUND SIX of a visual-polish convergence audit of the PI WEB client (Lit web app).',
...
const result = await runs.run('r6-lane-c-retry', { agent: 'opus-design-reviewer-b', label: 'opus round6 retry', timeoutMs: 5400000, task: lanePrompt(
    'a full pass over all 13 surfaces with a polish lens. Budget your time: read the source first and reserve live measurement for the handful of claims that need it. A shorter report with verified file:line beats an exhaustive probe run that never finishes.',
    'src/client/src/components (whole tree), pi-web-plugins (browser surfaces)',
    SURFACES,
    '/tmp/uiux-r6-lane-c.md'
  ), output: '/tmp/uiux-r6-lane-c.md' });
return { ok: true, file: '/tmp/uiux-r6-lane-c.md' };
