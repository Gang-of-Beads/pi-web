var meta = { name: 'goal_round_b_layout', description: 'Goal convergence Round B: three glm lanes audit layout coordination (alignment, whitespace rhythm, button boundaries, centering, balance) on the live 8505 stack' };
...
var CONTEXT = [
  ...
  'The app is in "pro native" style: monospace UI face, near-black space-gray palette (#0b0d10 base, lightness-stepped surfaces), square corners 0-4px, flat (no shadows), hairline borders. Goal: layout COORDINATION convergence. This is Round B - a RE-REVIEW: Round A fixed border contrast (#3a424e), heading distribution, column edges (one reading edge), fold-button radius + insets, empty-state centering, Files toolbar floor, pressed states, tile edge, heading rhythm. VERIFY each fix reached the reader (measure it), then hunt what Round A missed. Prior findings are in docs/design/research/rounda-lane-{a,b,c}.md and docs/design/review-triage-goal-round-a.md.',
  '',
  ...
  '',
  'Every finding: file:line of the owning style, the two surfaces or elements compared (or the one element + its container), measured numbers, screenshot path (/tmp/roundb-*.png), and an explicit TRUE/FALSE adjudication. If a hunt item is clean, say so with the measurement that proves it.',
].join('\n');

var laneA = runs.run('lane-a', {
  agent: 'design-reviewer-d', timeoutMs: 5400000, label: 'roundA glm A phone', output: '/tmp/roundb-lane-a.md',
...
var laneB = runs.run('lane-b', {
  agent: 'design-reviewer-e', timeoutMs: 5400000, label: 'roundA glm B desktop+responsive', output: '/tmp/roundb-lane-b.md',
