 ...
 *
 * The plugin declares its own shape rather than importing the host's: this
 * feature is the plugin's, and a self-contained plugin cannot have its facts
 * defined by the shell it happens to run in.
 */
...
async function describeRun(runsDir: string, runId: string, artifacts: Map<string, RunArtifact>, running: RunningArtifact | undefined, now: number, parentActive: boolean): Promise<SubagentRunInfo | undefined> {
 ...
 */
function runStatus(artifact: RunArtifact | undefined, lastWriteMs: number | undefined, startedAtMs: number, now: number, parentActive: boolean): SubagentRunInfo["status"] {
