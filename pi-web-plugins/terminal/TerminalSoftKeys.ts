...
export class TerminalSoftKeys extends LitElement {
  ...
  static override styles = [css`
    ...
    .terminal-soft-keys button { box-sizing: border-box; display: inline-flex; align-items: center; gap: var(--pi-space-3); flex: 0 0 auto; max-width: none; min-height: var(--pi-control-height-comfort); border: 1px solid var(--pi-border); border-radius: var(--pi-radius-md); background: var(--pi-surface); color: var(--pi-text); padding: var(--pi-space-3) var(--pi-space-5); font: var(--pi-text-xs) var(--pi-font-mono, ui-monospace, SFMono-Regular, Menlo, Consolas, monospace); line-height: inherit; cursor: pointer; touch-action: pan-x; -webkit-touch-callout: none; user-select: none; }
