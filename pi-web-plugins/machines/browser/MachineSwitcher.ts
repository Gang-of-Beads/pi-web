...
export class MachineSwitcher extends LitElement implements KeyboardNavigableSection {
  ...
  private renderMachineOption(machine: NavMachineSnapshot): TemplateResult {
    ...
    return html`
      <div class=${`machine-option ${selected ? "selected" : ""} ${hasActions ? "" : "no-actions"}`}>
        ...
        >
          <span class="machine-option-name">${this.renderActivity(machine)}<span>${machine.name}</span><span class=${`machine-option-status ${status}`}><span class="dot" aria-hidden="true"></span>${machineStatusLabel(status)}</span></span>
          <small>${machine.kind === "local" ? "Local Pi Web" : machine.baseUrl ?? "Remote Pi Web"}</small>
        </button>
        ${hasActions ? html`
  ...
  static override styles = css`
    ...
    .machine-switcher-label { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: var(--pi-text-xs); font-weight: var(--pi-weight-semibold); line-height: 1.2; }
    ...
    .machine-option-status { flex: 0 0 auto; display: inline-flex; align-items: center; gap: var(--pi-space-2); color: var(--pi-muted); font-size: var(--pi-text-2xs); font-weight: var(--pi-weight-medium); }
    ...
    .machine-option-status.offline .dot, .machine-option-status.error .dot { background: var(--pi-danger); }
    ...
    .machine-option-actions { position: absolute; top: var(--pi-space-2); right: var(--pi-space-2); }
