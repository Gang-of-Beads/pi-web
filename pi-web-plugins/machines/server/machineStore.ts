...
/** The machines store file inside the host's data directory. */
export function machineStorePathIn(dataDir: string, env: NodeJS.ProcessEnv = process.env, cwd = process.cwd()): string {
  const configured = env["PI_WEB_MACHINES_FILE"];
  if (configured === undefined || configured === "") return join(dataDir, "machines.json");
