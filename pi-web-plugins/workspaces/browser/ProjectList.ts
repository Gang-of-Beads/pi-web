...
export class ProjectList extends LitElement implements KeyboardNavigableSection {
  ...
  private renderAdd() {
    ...
    return html`<button class="section-add" title="Add project" aria-label="Add project" @click=${(event: Event) => { event.stopPropagation(); this.onAdd?.(); }}><span class="section-add-glyph" aria-hidden="true">+</span><span class="section-add-label">Add project</span></button>`;
  }
  ...
  static override styles = [css`
    ...
    .load-retry { box-sizing: border-box; min-height: var(--pi-control-height); padding: 0 var(--pi-space-4); font-size: var(--pi-text-xs); }
    @media (pointer: coarse) { .load-retry { min-height: var(--pi-control-height-touch); } }
  `];
}
