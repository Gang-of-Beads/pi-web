...
- npm packages: latest `@gang-of-beads/pi-web`; Pi Coding Agent is resolved as PI WEB's npm peer dependency (newest compatible version) and the peer-provided `pi` binary is linked into the image.
...
| `PI_WEB_VERSION` | npm version/range for `@gang-of-beads/pi-web`; Pi Coding Agent resolves from PI WEB's npm peer dependency |
